...
- Campus Meetup for new physical transactions (legacy courier fields are retained only for historical compatibility)
...
```text
Checkout → Midtrans Sandbox → Verified payment → Funds held in escrow
        → Campus Meetup → Buyer shows 6-digit code → Seller verifies code
        → Done → Funds released to seller
```

Unpaid checkouts expire automatically and reserved stock is returned. Payment becomes authoritative only after the backend verifies the Midtrans notification and confirms its status server-to-server.
...
### Requirements
- **Node.js `^22.22.3 || ^24.15.0 || >=26.0.0`** (see package engines / `.nvmrc`) and npm
...
> ⚠️ **Never commit secrets.** `DATABASE_URL`, `JWT_SECRET`, `OTP_HASH_SECRET`, `SUPABASE_SERVICE_ROLE_KEY`, `BREVO_API_KEY`, and `MIDTRANS_SERVER_KEY` are backend-only. Only `EXPO_PUBLIC_*` variables may be used in the frontend.
...
│       ├── transactions/    Checkout, escrow, handover code, service deliverables
│       ├── payments/        Midtrans payment creation & verified webhook handling
...
│       ├── uploads/         Listing images & private deliverables
│       └── ...              activity, complaints, config, users
│
...
BMarket is an **academic project**. Buyer payments currently use **Midtrans Sandbox**, so no real funds are processed through the public demo.

- New physical transactions use **Campus Meetup only**.
- The old BMarket wallet payment endpoint is disabled; current buyer checkout uses Midtrans.
- Wallet ledger entries remain for escrow, refunds, seller payout, and audit accounting.
- Legacy courier fields remain only so historical records stay readable.
- PDDikti/NIM automatic identity lookup is **not implemented**.
- The Expo project supports web, Android, and iOS; the public deployment currently uses the Vercel web build.
